...
import com.elong.mobile.plugin.hr.EPluginActivityOverider;
...
public class ActivityGenerateUtil {
	...
	public static void createActivityDex(String superClassName,
			String targetClassName, File saveTo, String pkgName)
			throws IOException {
		byte[] dex = createActivityDex(superClassName, targetClassName, pkgName);
	...
	public static <S, D extends S> byte[] createActivityDex(
		...
		// 定义字段
		// private static final String _pluginId = ogiName;
		// private AssetManager asm;
		// private Resources res;
//		String pluginId = EPluginNameUtil.mergerName(pkgName, superClassName);
		declareFields(dexMaker, generatedType, superType, superClassName, superClassName);
		// 声明 默认构造方法
		...
		declareMethod_getComponentName(dexMaker, generatedType, superType,
				superClassName);
	...
	private static <S, D extends S> void declareFields(DexMaker dexMaker,
		...
		FieldId<D, Boolean> beforeOnCreate = generatedType.getField(
				TypeId.BOOLEAN, FIELD_mOnCreated);
	...
	private static <S, D extends S> void declareMethod_setTheme(
		...
		TypeId<EPluginActivityOverider> ActivityOverider = TypeId
				.get(EPluginActivityOverider.class);
		// static int ActivityOverider::getPlugActivityTheme(Activity
		// fromAct,String pluginId)
		MethodId<EPluginActivityOverider, Integer> methodOveride = ActivityOverider
				.getMethod(TypeId.INT, "getPlugActivityTheme",
		...
		/*
		 * int resId = paramThemeId; if( !OnCreated ){ resId =
		 * ActivityOverider.getPlugActivityTheme(this,pluginId); } if(resId!=0){
		 * super.setTheme(resId); }
		 */
		FieldId<D, Boolean> onCreated = generatedType.getField(TypeId.BOOLEAN,
				FIELD_mOnCreated);
		methodCode.iget(onCreated, lcoalonCreate, localThis);
		{
			...
			methodCode.compare(Comparison.NE, ifBeforeOncreate, lcoalonCreate,
					localFalse);
	...
	private static <S, D extends S> void declareMethod_attachBaseContext(
		...
		// codes:
		// Object [] rs = ActivitiOverrider.overrideAttachBaseContext(_pluginId,
		// activity, base);
		MethodId<EPluginActivityOverider, Object[]> methodOverride = ActivityOverider
				.getMethod(ObjArr, "overrideAttachBaseContext", TypeId.STRING,
						TypeId.get(Activity.class), Context);
		methodCode.invokeStatic(methodOverride, rsArr, pluginId, localThis,
				base);
		...
		MethodId<Context, Resources> methodGetResources = Context.getMethod(
	...
	private static <S, D extends S> void declareMethod_getIntent(
		...
		MethodId<S, Intent> superMethod = superType.getMethod(Intent,
				methodName);
		...
		MethodId<D, ComponentName> getComponent = generatedType.getMethod(
				ComponentName, "getComponentName");

		code.invokeVirtual(getComponent, localComp, localThis);

		MethodId<Intent, Intent> setComponent = Intent.getMethod(Intent,
	...
	private static <S, D extends S> void declareMethod_getPackageName(
			DexMaker dexMaker, TypeId<D> generatedType, String pkgName) {
	...
	private static <S, D extends S> void declareMethod_getComponentName(
		...
		TypeId<ComponentName> ComponentName = TypeId.get(ComponentName.class);
		MethodId<D, ComponentName> method = generatedType.getMethod(
				ComponentName, "getComponentName");
		Code methodCode = dexMaker.declare(method, PROTECTED);
		Local<String> pkg = methodCode.newLocal(TypeId.STRING);
		...
		MethodId<ComponentName, Void> comp_constructor = ComponentName
				.getConstructor(TypeId.STRING, TypeId.STRING);
	...
	private static <S, D extends S> void declareMethod_onCreate(
		...
		MethodId<EPluginActivityOverider, Void> method_call_onCreate = ActivityOverider
	...
	private static <S, D extends S> void declareMethod_startActivityForResult(
		...
		TypeId<EPluginActivityOverider> ActivityOverider = TypeId
				.get(EPluginActivityOverider.class);
		MethodId<EPluginActivityOverider, Intent> methodOveride = ActivityOverider
	...
	private static <S, D extends S> void declareMethod_onBackPressed(
		...
		MethodId<EPluginActivityOverider, Boolean> methodOveride = ActivityOverider
	...
	private static <S, D extends S> void declareMethod_startService(
		...
		MethodId<D, ComponentName> method = generatedType.getMethod(returnType,
				"startService", Intent);
		MethodId<EPluginActivityOverider, ComponentName> methodOveride = ActivityOverider
	...
	private static <S, D extends S> void declareMethod_unbindService(
		...
		MethodId<D, Void> method = generatedType.getMethod(TypeId.VOID,
				"unbindService", Conn);
		MethodId<EPluginActivityOverider, Void> methodOveride = ActivityOverider
