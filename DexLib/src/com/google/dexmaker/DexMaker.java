 ...
 * in your Android application, you can dynamically generate and load executable
 * code. This approach takes advantage of the fact that the host environment and
 * target environment are both Android.
 * <li><strong>For compile time code generation.</strong> You may use this
 * library as a part of a compiler that targets Android. In this scenario the
 * generated dex file must be installed on an Android device before it can be
 * executed.
 * </ul>
 *
 * <h3>Example: Fibonacci</h3>
 * To illustrate how this API is used, we'll use DexMaker to generate a class
 * equivalent to the following Java source:
 * 
 * <pre>
 * {@code
 * 
 ...
 *
 * <p>
 * We start by creating a {@link TypeId} to identify the generated
 * {@code Fibonacci} class. DexMaker identifies types by their internal names
 * like {@code Ljava/lang/Object;} rather than their Java identifiers like
 ...
 *
 * <p>
 * Next we declare the class. It allows us to specify the type's source file for
 * stack traces, its modifiers, its superclass, and the interfaces it
 ...
 * variables to be created before any instructions are emitted. Use
 * {@link Code#newLocal newLocal()} to create a new local variable. The method's
 ...
 * variables that we'll need for our {@code fib()} method:
 * 
 * <pre>
 * {
 * 	&#064;code
 ...
 *
 * <p>
 * We're done defining the dex file. We just need to write it to the filesystem
 * or load it into the current process. For this example we'll load the
 * generated code into the current process. This only works when the current
 ...
 * loader and invoke its {@code fib()} method:
 * 
 * <pre>
 * {
 * 	&#064;code
 * 	Class&lt;?&gt; fibonacciClass = loader
 * 			.loadClass(&quot;com.google.dexmaker.examples.Fibonacci&quot;);
...
public final class DexMaker {
	 ...
	 *
	 * @param flags
	 *            a bitwise combination of {@link Modifier#PUBLIC},
	 *            {@link Modifier#FINAL} and {@link Modifier#ABSTRACT}.
	 */
	public void declare(TypeId<?> type, String sourceFile, int flags,
		...
		int supportedFlags = Modifier.PUBLIC | Modifier.FINAL
				| Modifier.ABSTRACT;
	 ...
	 *            {@link Modifier#STATIC}, {@link Modifier#FINAL} and
	 *            {@link Modifier#SYNCHRONIZED}.
	 *            <p>
	 *            <strong>Warning:</strong> the {@link Modifier#SYNCHRONIZED}
	 *            flag is insufficient to generate a synchronized method. You
	 *            must also use {@link Code#monitorEnter} and
	 *            {@link Code#monitorExit} to acquire a monitor.
	 */
	public Code declare(MethodId<?, ?> method, int flags) {
		...
		if ((flags & Modifier.SYNCHRONIZED) != 0) {
			flags = (flags & ~Modifier.SYNCHRONIZED)
	 ...
	 *            {@link Modifier#VOLATILE}, and {@link Modifier#TRANSIENT}.
	 * @param staticValue
	 *            a constant representing the initial value for the static
	 *            field, possibly null. This must be null if this field is
	 *            non-static.
	 */
	public void declare(FieldId<?, ?> fieldId, int flags, Object staticValue) {
		...
		if ((flags & Modifier.STATIC) == 0 && staticValue != null) {
			throw new IllegalArgumentException(
					"staticValue is non-null, but field is not static");
		}
	 ...
	 *
	 * <h3>Picking a dex cache directory</h3> The {@code dexCache} should be an
	 * application-private directory. If you pass a world-writable directory
	 * like {@code /sdcard} a malicious app could inject code into your process.
	 ...
	 * </pre>
	 * 
	 * If the {@code dexCache} is null, this method will consult the
	 * {@code dexmaker.dexcache} system property. If that exists, it will be
	 * used for the dex cache. If it doesn't exist, this method will attempt to
	 * guess the application's private data directory as a last resort. If that
	 * fails, this method will fail with an unchecked exception. You can avoid
	 * the exception by either providing a non-null value or setting the system
	 * property.
	 *
	 * @param parent
	 *            the parent ClassLoader to be used when loading our generated
	 *            types
	 * @param dexCache
	 *            the destination directory where generated and optimized dex
	 *            files will be written. If null, this class will try to guess
	 *            the application's private data dir.
	 */
	public ClassLoader generateAndLoad(ClassLoader parent, File dexCache)
			throws IOException {
		if (dexCache == null) {
		...
		try {
			return (ClassLoader) Class
					...
					.getConstructor(String.class, String.class, String.class,
							ClassLoader.class)
					.newInstance(result.getPath(), dexCache.getAbsolutePath(),
							null, parent);
	...
	public void generateDEX(File dexCache) throws Exception {
		byte[] dex = generate();
		File result = File.createTempFile("Generated", ".jar", dexCache);
		result.deleteOnExit();
		JarOutputStream jarOut = new JarOutputStream(new FileOutputStream(
